...
CTR.SolariSegment = function( settings ) {
	...
	function _init() {
		...
		_li = document.createElement( 'span' );
...
CTR.SolariBoard = function( settings ) {
	...
	var
		_settings = settings,
		_w = 0,
		_segments = document.createElement( 'td' ),
		_segmentArray = []
	;
	...
	function _init() {
		...
		for( var j = 0; j < _segmentArray.length; j++ ) {
			if( j < _segmentArray.length - 1 ) {
				...
				_segmentArray[ j ].getElement().style.marginRight = '6px';
				_w += 6;
			}
		}
		//_segments.style.width = _w + 60 + 'px';
		//_segments.style.height = _settings.segmentHeight + 'px';
...
CTR.SolariTable = function( settings ) {
	...
	function _setContent( j, v ) {
		for (i = 0; i < v.length; i++)
			_tableArray[j][i].setContent(v[i]);
	}
	...
	function _init() {
		...
		//console.debug(_settings.format);
		for( var j = 0; j < _settings.format.length; j++ ) {
			var lineArray = [], line = document.createElement('tr'), tw = 0;
			for (var i = 0; i < _settings.format[j].length; i++ ) {
				var elm = new CTR.SolariBoard({ format: _settings.format[j][i], 
												segmentWidth: _settings.segmentWidth,
												segmentHeight: _settings.segmentHeight,
												fontsize: _settings.fontSize } );
				lineArray.push( elm );
				line.appendChild( elm.getDisplay() );
				tw += elm.w;
			}
			_tableArray.push( lineArray );
			_table.appendChild( line );
			w = Math.max(w, tw);
		}
		
		var h = _settings.format.length * _settings.segmentHeight;
		
		_display.style.width = w + 60 + 'px';
	...
	function _update() {
		...
		for( var j = 0; j < _tableArray.length; j++ ) {
			for (var i = 0; i < _tableArray[j].length; i++ ) {
				_tableArray[j][i].update();
			}
		}
